...
<template>
  ...
  >
    <nav class="container-page flex items-center justify-between h-16 lg:h-20" aria-label="Primary">
      <!-- Logo -->
      <NuxtLink to="/" class="flex items-center gap-2.5 group" aria-label="Indian Mentors home">
        <span
          class="relative grid place-items-center w-9 h-9 rounded-xl bg-cta-gradient shadow-cta text-white font-extrabold text-sm transition-transform duration-300 group-hover:-rotate-6 group-hover:scale-105"
        >
          IM
          <span class="absolute -inset-1 rounded-2xl bg-blue-400/30 blur-md -z-10 opacity-0 group-hover:opacity-100 transition-opacity"></span>
        </span>
      ...
      <div class="hidden lg:flex items-center gap-3">
        <a
          href="#login"
          class="text-sm font-medium text-slate-700 hover:text-blue-700 transition-colors px-3 py-2"
        >
          Sign in
        </a>
      ...
      <button
        type="button"
        class="lg:hidden p-2.5 rounded-xl text-slate-700 hover:bg-slate-100 transition-colors"
